...
// =======================================================
const API_URL = "https://script.google.com/macros/s/AKfycbxPSgHbJ7BnEezl4H5rfIyjmDxSg9HOfSMK4EDKxp4Bc1nBEVGZP1WvF_cpDJXt7QA/exec"; 
// ------------------------------------------------
...
let currentFilters = { status: 'all', priority: 'all' }; // Filtros iniciais
let allRequests = []; 
...
async function fetchAndRenderRequests() {
    ...
    try {
        ...
        // CORREÇÃO CRÍTICA: Se a API não estiver com CORS configurado, 
        // a linha a seguir falhará ou retornará dados vazios.
        if (!response.ok) throw new Error("Erro de rede ou a API retornou um status não-OK.");
        
        const data = await response.json();
        // A API precisa retornar um objeto com a chave 'solicitacoes'
        ...
        requests.forEach(req => { 
            ...
            // Use o 'motivo' para priorizar, caso exista
            ...
            // Garante que detalhesDeficiencia é um array para evitar erro no frontend
            if (typeof req.detalhesDeficiencia === 'string') {
                req.detalhesDeficiencia = req.detalhesDeficiencia.split(',').map(s => s.trim()).filter(Boolean);
            }
        });
        
        allRequests = requests;
        
        // Aplica os filtros e renderiza (CORRIGIDO)
        ...
        if (currentFilters.priority !== 'all') {
            filteredRequests = filteredRequests.filter(req => req.prioridade === currentFilters.priority);
        }
        ...
        requestList.innerHTML = '<p style="grid-column: 1 / -1; text-align: center; color: #dc3545; font-weight: bold;">Erro de conexão com a API. Verifique a URL e o CORS no Apps Script.</p>';
    }
}
...
function showRequestDetails(req) {
    ...
    // Mostra os detalhes da deficiência
    const deficienciaDetalhes = Array.isArray(req.detalhesDeficiencia) ? req.detalhesDeficiencia.join(', ') : (req.detalhesDeficiencia || 'Não possui');
    document.getElementById('modalDeficiencia').textContent = deficienciaDetalhes;
...
document.querySelectorAll('.filter-btn').forEach(btn => btn.addEventListener('click', function() {
    ...
    // Atualiza o filtro correto (status ou priority)
...
window.addEventListener('storage-update', fetchAndRenderRequests); // Listener para o site de pedidosv